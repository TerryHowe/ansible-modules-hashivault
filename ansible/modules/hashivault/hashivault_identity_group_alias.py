...
options:
    ...
    authtype:
        description:
            - "authentication type to use: token, userpass, github, ldap, approle, oidc"
        default: token
    token:
        description:
            - token for vault
    ...
    state:
        description:
            - whether create/update or delete the group
'''
EXAMPLES = '''
---
- hosts: localhost
    ...
    try:
        ...
        if alias_id is not None:
            if alias_id not in list_of_aliases['data']['keys']:
                return {'changed': False}
            client.secrets.identity.delete_group_alias(alias_id)
            return {'changed': True}
            ...
            for key, value in dict(list_of_aliases['data']['key_info']).items():
                if value['mount_accessor'] == mount_accessor and \
                    ...
                    client.secrets.identity.delete_group_alias(key)
                    return {'changed': True}
            return {'changed': False}
